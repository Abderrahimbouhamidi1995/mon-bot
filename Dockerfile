...
# ---- Test Stage ----
    ...
    EXPOSE 5005
    ENTRYPOINT []
    CMD ["bash", "-c", "echo 'Starting training...'; rasa train && echo 'Launching Rasa server on port 5005...' && rasa run --port 5005 --enable-api --cors '*' --debug & PID=$!; echo 'Waiting for Rasa server to be ready...' && until curl -s http://localhost:5005/status; do sleep 5; done; echo 'Server is up, running tests...' && robot tests/utterances_tests.robot; echo 'Tests finished, stopping Rasa server...' && kill $PID; if grep -Fq 'Incorrect' /app/tests/test_results.csv; then echo 'Test results contain Incorrect. Eliminating last changes...'; git config --global --add safe.directory /app && git config --global user.email \"a.bouhamidi@outlook.com\" && git config --global user.name \"Abderrahimbouhamidi1995\" && git remote set-url origin https://$PAT_TOKEN@github.com/Abderrahimbouhamidi1995/mon-bot.git && git reset --hard HEAD~1 && git push origin HEAD --force; exit 1; else echo 'All tests passed.'; exit 0; fi"]
    # ---- Production Stage ----
    FROM base AS prod
    EXPOSE 5005
